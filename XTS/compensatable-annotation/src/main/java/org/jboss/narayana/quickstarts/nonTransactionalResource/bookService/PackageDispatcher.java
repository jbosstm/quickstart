...
public class PackageDispatcher {
    ...
    public static Set<String> availableItems = new HashSet<String>();

    @TxCompensate(RecallPackage.class)
    @TxConfirm(ConfirmPackage.class)
    public void dispatch(String item, String address) {
        if (!availableItems.contains(item)) {
            throw new PackageDispatcherException("Requested item is not available");
        }
