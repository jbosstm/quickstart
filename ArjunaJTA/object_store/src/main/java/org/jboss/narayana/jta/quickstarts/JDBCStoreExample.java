...
public class JDBCStoreExample {
    ...
    public static void enlistResources() throws SystemException, RollbackException {
        ...
        // create two resource, although they won't actually do anything, they will force a log record to be created
    ...
    public static void setupStoreViaConfigBean() {
        ...
        // Narayana has three types of store for persisting different categories of information,
        // and we will want to use a database for each of them:
        ...
        defaultActionStoreObjectStoreEnvironmentBean.setObjectStoreType(jdbcStoreClass);
        stateStoreObjectStoreEnvironmentBean.setObjectStoreType(jdbcStoreClass);
        communicationStoreObjectStoreEnvironmentBean.setObjectStoreType(jdbcStoreClass);

        // set the JNDI name of the datasource to be  used to access the database
        defaultActionStoreObjectStoreEnvironmentBean.setJdbcAccess(dataSourceJndiName);
        stateStoreObjectStoreEnvironmentBean.setJdbcAccess(dataSourceJndiName);
        communicationStoreObjectStoreEnvironmentBean.setJdbcAccess(dataSourceJndiName);
    }

    public static void setupStoreViaPropertiesFile() {
        // the same config can be specified using jbossts propertie files:
